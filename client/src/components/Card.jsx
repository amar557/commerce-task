...
function Card({ product }) {
  ...
  console.log();
  return (
    <div className=" card cursor-grab">
      <div className=" transition-all duration-300 relative md:h-80 h-40   overflow-hidden">
        <img
          src={product.image}
          alt=""
        ...
        <div className=" absolute top-2 right-2    opacity-0 card-heart ">
          ...
          <div className="absolute top-10 -left-5 p-1 capitalize rounded-md px-2  text-sm opacity-0 suggestion-w bg-black text-white ">
            add to wishlist
          </div>
        </div>
      </div>
      <div className="py-1 text-center space-y-1">
        <h3 className="text-center font-medium  uppercase text-sm  md:text-base">
          {product.title}
        </h3>
        <p className="sm:text-sm text-center md:leading-normal leading-tight text-xs ">
          {desc.slice(0, 9).join(" ")}
          {desc.length > 9 ? "..." : ""}
        </p>
